// Modal iframe overlay for the full DoubleCheck review interface
...
export function openReviewModal(wiki: string, revId: number): void {
  ...
  // Send init info and set up postMessage bridge once iframe loads
...
// ---- PostMessage bridge: handles Wikipedia API actions from the iframe ----
...
// ---- Wikipedia API helpers (same-origin, uses user's session cookies) ----

function getWikiUser(): { username: string | null; groups: string[] } | null {
  try {
    const mw = (window as Record<string, unknown>).mw as
...
async function performRevert(
  ...
  // Verify we're on the correct wiki
  ...
  try {
    ...
    // Look up page title for the revision
...
async function performWarn(
  ...
  try {
    ...
    const sectionTitle = `Warning: Editing on [[${articleTitle}]]`;

    const form = new URLSearchParams();
    form.set("action", "edit");
    form.set("title", talkPage);
    form.set("section", "new");
    form.set("sectiontitle", sectionTitle);
